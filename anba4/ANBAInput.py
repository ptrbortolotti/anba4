...
def writeResultYaml(anba,fileName):
    outFile = open(fileName,'wt')

    ## Compute stiffness and add results to dictionary    
    stiff = anba.compute()
    outFile.write('stiffness: [')
    for i in range(0,6):
        for j in range(i,6):
            outFile.write(str(stiff.getValue(i,j)))
            if(i < 5 or j < 5):
                outFile.write(', ')
    outFile.write(']\n')
    
    ## Compute mass and add results to dictionary
    mass = anba.inertia()
    outFile.write('mass: [')
    for i in range(0,6):
        for j in range(i,6):
    ...
    ## Attempt to add stress and strain results to dictionary
    try:
        vSts = anba.STRESS.compute_vertex_values()
        numPts = int(len(vSts)/6)
        outFile.write('stress:\n')
        for i in range(0,numPts):
            outFile.write('    - [')
            for j in range(0,6):
                outFile.write(str(vSts[i+j*numPts]))
                if(j < 5):
                    outFile.write(', ')
            outFile.write(']\n')
    except:
        print('An error occured constructing the point by point stress field for yaml output.  Omitting stress from output')
        
    try:
        vStn = anba.STRAIN.compute_vertex_values()
        numPts = int(len(vStn)/6)
        outFile.write('strain:\n')
        for i in range(0,numPts):
            outFile.write('    - [')
            for j in range(0,6):
                outFile.write(str(vStn[i+j*numPts]))
                if(j < 5):
                    outFile.write(', ')
            outFile.write(']\n')
    except:
        print('An error occured constructing the point by point strain field for yaml output.  Omitting strain from output')
